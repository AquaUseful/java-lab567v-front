...
<div class="mb-3">
<label for="orderProduct${order.id}" class="form-label fw-bold">Наименование товара</label>
<input class="form-control" type="text" id="orderProduct${order.id}" name="product" required value="${order.product}">
</div>
<div class="mb-3">
<label for="orderAddress${order.id}" class="form-label fw-bold">Адрес доставки</label>
<input class="form-control" type="text" id="orderAddress${order.id}" name="address" required value="${order.address}">
</div>
<div class="mb-3">
<label for="orderComment${order.id}" class="form-label fw-bold">Комментарий</label>
